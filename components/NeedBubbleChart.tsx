...
interface NeedBubbleChartProps {
  data: NeedBubbleData[];
...
export function NeedBubbleChart({ data, config, callbacks, loading = false }: NeedBubbleChartProps) {
  ...
  const [bubbles, setBubbles] = useState<NeedBubbleData[]>([]);
  ...
  const updateBubbles = useCallback((newBubbles: NeedBubbleData[]) => {
    const now = Date.now();
  ...
  useEffect(() => {
    ...
    const centerPullForce = () => {
      initialBubbles.forEach(bubble => {
        ...
        // Stronger pull for larger bubbles (higher frequency needs)
  ...
  // Handle bubble press
  const handleBubblePress = (bubble: NeedBubbleData) => {
  ...
  const handleBubbleLongPress = (bubble: NeedBubbleData) => {
    ...
    return (
      <View style={[styles.container, styles.emptyContainer]}>
        ...
        <Text style={[styles.emptySubtext, { color: isDark ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.4)' }]}>
          Start a conversation to understand your needs
        </Text>
      </View>
    );
  }
