...
interface PartBubbleChartProps {
  data: PartBubbleData[];
...
export function PartBubbleChart({ data, config, callbacks, loading = false }: PartBubbleChartProps) {
  ...
  const [bubbles, setBubbles] = useState<PartBubbleData[]>([]);
  ...
  const updateBubbles = useCallback((newBubbles: PartBubbleData[]) => {
    const now = Date.now();
  ...
  useEffect(() => {
    ...
    const centerPullForce = () => {
      initialBubbles.forEach(bubble => {
        ...
        // Stronger pull for larger bubbles (higher frequency parts)
  ...
  // Handle bubble press
  const handleBubblePress = (bubble: PartBubbleData) => {
  ...
  const handleBubbleLongPress = (bubble: PartBubbleData) => {
    ...
    return (
      <View style={[styles.container, styles.emptyContainer]}>
        ...
        <Text style={[styles.emptySubtext, { color: isDark ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.4)' }]}>
          Start a conversation to explore your inner parts
        </Text>
      </View>
    );
  }
            ...
            return (
              ...
              >
                <View style={{
                  ...
                  {/* Bubble label */}
                  {(() => {
                    const fontSize = getFontSize(bubble.radius, bubble.name);
                    const textLines = wrapTextForBubble(bubble.name, bubble.radius, fontSize);
